...
func networkLisener(gs *GameState){
	...
	func parseServerUpdate(gs *GameState, g *ClientGame, payload []byte) {
		// DEBUG: In toàn bộ bytes nhận được
		// fmt.Printf("RECEIVED HEX: ")
		...
		// if !
		gs.receiver.OnPacketReceived(packetSeq)
		//  {
			// return
		// }
		// fmt.Println("packet seq ",packetSeq)
		...
		if reader.HasMore() {
			...
			for i := 0; i < int(eventCount) && reader.HasMore(); i++ {
				...
				// fmt.Println("nhan event type ", evType, " payload len ", payloadLen, " payload ", eventPayload)
...
func sendAckEvents(gs *GameState, g *ClientGame){
	...
	// fmt.Println("send ack" , "higest ", highestSeq, " mask ", mask)
